...
def main() -> int:
    ...
    task_prompt = base_prompt + (
        ...
        "- 在每个任务描述里**显式写出**该任务依赖的 skill `[[skill 文件名]]` wikilink\n"
        "- skill wikilink **只能从上文 §6 工程参考 skill 索引（F-技术主管 / F-后端 / F-前端 / F-架构）中挑**\n"
        "- 命名规约：`A<N>-标题` 架构师域 / `TL<N>-标题` 技术主管域 / `B<N>-标题` 后端域 / `F<N>-标题` 前端域\n"
        "- **禁止编造文件名**——不在索引里出现的 wikilink 一律不写\n"
        "- 不确定时优先不写 wikilink（下游有 keyword 触发器兜底）\n"
        "- 用法示例：`任务 T01 实现用户登录 API，依赖 [[B7-FastAPI共享资源async]] + [[B5-空集守卫]]`\n\n"
        "**禁止在给技术主管文件中出现**：\n"
        "- 架构图（Mermaid / 文字图）\n"

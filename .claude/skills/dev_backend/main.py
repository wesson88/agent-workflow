...
def main() -> int:
    ...
    for task_file in task_files:
        ...
        # required 清单只放路径**示例**（取 技术栈.md §5 的默认布局——改默认值
        # 请改那份规则并同步这里）；实际路径以指令清单声明的为准，见 user_prompt。
        # API契约 是必产文件，走 _render_api_contract_mandate 单独强调（不与示例混排）
        required = [
            "src/backend/main.py",
        ...
        user_prompt = (
            ...
            "请按指令清单完整实现后端：\n"
            # 2026-08-16：原为硬编码 `src/backend/` + 无条件鉴权要求。二者都是
            # **项目级架构决策**，写死在跨项目模板里会与项目 spec 对撞——
            # huashu-demo T06 实战即因此产出孤儿文件（见 技术栈.md §5.2）。
            # 现改为「引用 canonical 规则 + 项目 spec 优先」，默认值统一由
            # 技术栈.md §5/§6 提供（该文件已全文注入 context，四个 SE 角色都读得到）。
            "  - **代码与测试路径**：以本轮指令清单显式声明的路径为准；"
            "指令未声明时按 `技术栈.md §5` 默认布局。若两者冲突，"
            "以指令清单为准并在产出中显式说明依据，不要静默二选一。\n"
            f"{api_hint}"
            "技术栈严格按 `00-系统/规则/se/技术栈.md`，禁止引入未授权依赖。\n"
            "  - **API 横切要求**：默认所有 API 含输入校验、鉴权、结构化日志"
            "（`技术栈.md §6`）。**例外**：当 PRD / 系统设计显式排除某项时按项目决策执行，"
            "并在 `API契约.md` 对应小节写明理由（如「无鉴权 —— 架构决策，见系统设计」），"
            "不得擅自加回、不得静默留空、不得杜撰方案。\n"

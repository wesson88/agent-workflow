...
def main() -> int:
    ...
    user_prompt = (
        ...
        "   - **章节结构严格按 [[产物schema#4. 曲作.md（作曲产出）]]「必填章节」模板 verbatim 输出**：\n"
        "     §1 旋律设计概述 / §2 和弦走向 / §3 旋律 motif / §4 段落对比设计 / §5 调性 / 调式 / 节奏 / §6 流派 idiom 来源标注 / §7 与 [[创作 vision]] 的对应\n"
        "   - **§6 流派 idiom 来源必须以 `[[F-{流派名}]]` wikilink 标注**（如 `[[F-民谣]]` / `[[F-雷鬼]]`），不可用裸汉字\n"
        "   - 给下游编曲 / 和声编写消费的乐理层文档\n"
        "   - 按词作的段落结构对齐（同 section 数）\n\n"
        "2. `Suno-prompt.md` — Suno 双层架构产物：\n"
        "   - **章节结构严格按 [[产物schema#5. Suno-prompt.md（作曲产出，附产物）]]「必填章节」模板 verbatim 输出**：\n"
        "     §1 主 prompt / §2 建议参数 / §3 歌词输入 / §4 执行指引 / §5 备用 prompt / §6 验收 checklist\n"

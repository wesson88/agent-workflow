"""
music_vocal_arranger/main.py — 和声编写执行入口（音乐域 L3）

输入（vault，来源：角色 frontmatter `inputs` 字段）：
  - 10-项目/music/{project}/指令/给和声编写.md
...
输出（vault，来源：角色 frontmatter `outputs` 字段）：
  ...
  - 10-项目/music/{project}/和声-Suno补丁.md

CLI：
  python .claude/skills/music_vocal_arranger/main.py --task "..." --project myproj
"""
...
def main() -> int:
    ...
    # 上游硬约束：指令/给和声编写.md + 曲作.md
    required_upstream = {"给和声编写.md", "曲作.md"}
    missing = [
    ...
    user_prompt = (
        ...
        f"{context}\n\n---\n"
        f"本轮和声编写诉求：{task or '（未提供，请基于上游曲作 + 词作 + Suno-prompt 综合推导）'}\n\n"
        "作为和声编写，请同时产出 **2 份** FILE 块：\n\n"
        "1. `和声谱.md` — 和声层次 / 三度六度铺设 / call-response 设计 / 假声切换 / 人声 stack\n"
        "   - 服务主旋律，**禁止喧宾夺主**\n"
        "   - 按曲作的段落结构对齐（同 section 数 / 同调性）\n"
        "   - 不越界写编曲 / 作曲 / 混音层决策\n\n"
        "2. `和声-Suno补丁.md` — 针对 Suno-prompt.md 已有 Lyrics inline tag 的**和声层增补建议**：\n"
        "   - 例：B 段 chorus 副歌处增加 `[harmony enters: third interval female-style backing]` 类 inline tag\n"
        "   - 若主旋律已足够、不需和声补丁，可声明「无需 patch」\n"
